 ...
 */
public class ProductButtonEditor extends DefaultCellEditor {
    ...
    public ProductButtonEditor(JFrame frame,JScrollPane scrollPane,JButton reloadButton) {
    ...
    public Component getTableCellEditorComponent(final JTable table, Object value, boolean isSelected, int row, final int column) {
        ...
        this.button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
            	...
            	ProductButtonEditor.this.fireEditingCanceled();
            	if(count<1){
            		// 可以将table传入，通过getSelectedRow,getSelectColumn方法获取到当前选择的行和列及其它操作等。  
            		int selectRow=table.getSelectedRow();
            		String sid=table.getValueAt(selectRow, 7).toString();
            		String cName=table.getValueAt(selectRow, 1).toString();
            		if(column==5){
            			//获取编辑产品对话框模型
            			dlg = new ProductAddDialog(frame, true,"编辑产品信息",Integer.parseInt(sid),reloadButton);
            		}else{
            			//获取删除对话框模型
            			dlg = new MessageDialog(frame, true,Integer.parseInt(sid),cName,reloadButton,"panel4");
            		}
